...
class _InvoiceViewerScreenState extends State<InvoiceViewerScreen> {
  ...
  Future<void> _loadInvoice() async {
    ...
    try {
      final filePath = await _invoiceService.fetchInvoiceForViewing(
      ...
      if (filePath != null) {
        ...
        if (mounted) {
          setState(() {
            _isLoading = false;
            _hasError = true;
            _errorMessage = 'Failed to load invoice. Please try again.';
          });
        }
      }
    } catch (e) {
  ...
  Future<void> _downloadInvoice() async {
    ...
    try {
      final fileName = 'invoice_${widget.appointmentId}.pdf';
      
      final success = await _invoiceService.downloadInvoicePermanently(
        appointmentId: widget.appointmentId,
        fileName: fileName,
        onProgress: (received, total) {
          if (total != -1 && mounted) {
            ...
            SnackBar(
              content: const Row(
                children: [
                  ...
                  SizedBox(width: 12),
                  Expanded(
                    child: Text('Invoice downloaded successfully'),
                  ),
                ],
              ),
            ...
            SnackBar(
              content: const Row(
                children: [
                  ...
                  SizedBox(width: 12),
                  Expanded(
                    child: Text('Failed to download invoice'),
                  ),
                ],
              ),
  ...
  Future<void> _shareInvoice() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        [xFile],
        subject: 'Invoice - Dr. ${widget.doctorName}',
        text: 'Sharing invoice from Dr. ${widget.doctorName}',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(
                  child: Text('Failed to share invoice: ${e.toString()}'),
                ),
              ],
            ),
            backgroundColor: AppColors.errorRed,
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: _isDownloading || _localFilePath == null ? null : _downloadInvoice,
              tooltip: 'Download',
            ),
            IconButton(
              icon: Icon(
              ...
              onPressed: _localFilePath == null ? null : _shareInvoice,
              tooltip: 'Share',
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _errorMessage ?? 'There was a problem loading the invoice. Please try again.',
